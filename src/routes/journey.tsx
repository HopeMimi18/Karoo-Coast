...
export const Route = createFileRoute("/journey")({
  head: () => ({
    meta: [
      { title: "Live ride — Pretoria to Cape Town on the Cape main line" },
      {
        name: "description",
        content:
...
function Journey() {
  ...
  const collect = useCallback(async () => {
    if (!user || stamps.includes(active.id)) return;
    setStamps((prev) => [...prev, active.id]);
    await supabase.from("stamps").insert({ user_id: user.id, stop_id: active.id });
  }, [user, stamps, active.id]);
      ...
      <header className="pointer-events-none absolute inset-x-0 top-0 z-[1200] flex flex-wrap items-start justify-between gap-3 p-4 sm:p-5">
        <div className="pointer-events-auto rounded-sm border border-border bg-card/85 px-4 py-3 backdrop-blur-md">
          ...
          <nav className="mt-2 flex gap-3 text-[11px]">
            ...
            <Link to="/passport" className="text-primary underline-offset-4 hover:underline">
              passport
            </Link>
          ...
          <div className="mt-4 flex flex-wrap items-center gap-2">
            {user ? (
              <button
                ...
                disabled={stamps.includes(active.id)}
                className={`rounded-sm border px-3 py-1.5 text-[11px] ${
                  stamps.includes(active.id)
                    ? "border-thread-name text-thread-name"
                    : "border-primary text-primary hover:bg-primary hover:text-primary-foreground"
                }`}
              >
                {stamps.includes(active.id) ? "✓ stamped" : `Stamp ${active.name}`}
              </button>
            ) : (
              <Link to="/auth" className="rounded-sm border border-border px-3 py-1.5 text-[11px] text-muted-foreground hover:text-foreground">
                Sign in to stamp this stop
              </Link>
            )}
            <Link
              to="/stories"
              className="rounded-sm border border-border px-3 py-1.5 text-[11px] text-muted-foreground hover:text-foreground"
            >
              Community stories
            </Link>
          </div>
                ...
                <li key={s.id}>
                  ...
                  >
                    <span className="truncate">
                      {stamps.includes(s.id) && <span className="text-thread-name">✓ </span>}
                      {s.name}
                    </span>
